...
public class SerialChannelTest extends Assert {
    ...
    @Before
    public void init() {
        channel = new SerialEventChannel(10, executor);
    ...
    public synchronized void waitingTimeout() throws InterruptedException {
        ...
        wait(10000L);
        assertEquals(expectedInt, events.get(0));
        while(!SerialEventChannel.State.PROCESSING_WAITING.equals(MockUtils.getFieldValue(channel, "state"))) {
